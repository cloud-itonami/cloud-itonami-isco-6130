(ns mixed-farming.advisor
  "FarmOpsAdvisor — proposes a mixed-farming operation (crop treatment,
  livestock feeding, harvest) for a plot. The advisor is swappable:
  ...
  and has no notion of plot provenance or safety; `mixed-farming.governor`
  is the independent system that decides whether the proposal may
  proceed, per the itonami actor pattern.

  A proposal is a map:
    {:op :treat-crop|:feed-livestock|:harvest
...
(defn- infer
  ...
  {:op op
   ...
   :rationale (str "proposed " (name op) " for plot " (:plot-id request))})
...
(def ^:private system-prompt
  "You are a farm-operations advisor. Given a mixed-crop-and-animal-producer
