(ns mixed-farming.actor
  "FarmOpsActor — the ISCO-08 6130 independent mixed-crop-and-animal-
  producer actor as a `langgraph.graph/state-graph` (per ADR-2607011000
  / CLAUDE.md Actors section). One graph run = one farm operation
  ...
  The unconditional invariant: the FarmOpsAdvisor can never directly
  commit a record or dispatch a robot action the FarmOpsGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [mixed-farming.advisor :as advisor]
            [mixed-farming.governor :as governor]
            [mixed-farming.store :as store]))

(defn build-graph
  "Build a compiled FarmOpsActor graph. `store` implements
  `mixed-farming.store/Store`. `advisor` implements
  `mixed-farming.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:plot-id (:plot-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
